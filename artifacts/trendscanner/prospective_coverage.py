...
def record_complete_candle(database_url: str, *, shadow_version: str,
                           ...
                           timeframe: str = "4h") -> dict:
    """Append one complete candle and report any missing 4h intervals."""
    ...
    with _connect_postgres(database_url) as connection:
        with connection.cursor() as cur:
            ...
            if timestamp == previous:
                return {"status": "DUPLICATE", "ready_timestamp": timestamp,
                        "previous_timestamp": previous, "gap_candles": 0}
            if timestamp < previous:
